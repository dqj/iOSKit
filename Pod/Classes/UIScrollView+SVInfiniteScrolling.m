...
//        if (self.needRefreshState)
//            self.scrollView.showsInfiniteScrolling = (self.scrollView.frame.size.height <= self.scrollView.contentSize.height);
    }
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        ...
        ZNLog(@"%@ %f %f %d", NSStringFromCGPoint(contentOffset), scrollOffsetThreshold, scrollViewContentHeight, self.state);
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {    
    ...
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        CGFloat scrollViewContentHeight = self.scrollView.contentSize.height;
        CGFloat scrollOffsetThreshold = scrollViewContentHeight-self.scrollView.bounds.size.height+SVInfiniteScrollingViewHeight/2.;
        
        if(contentOffset.y > scrollOffsetThreshold && (self.scrollView.isDragging || self.scrollView.isDecelerating || self.scrollView.isTracking) && self.scrollView.showsInfiniteScrolling) {
            self.state = SVInfiniteScrollingStateLoading;
        }
    }
}
