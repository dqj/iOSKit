...
#import "PullRefreshActivityView.h"
...
@property (nonatomic, copy) void (^dragProgressHandler)(CGFloat progress);
...
@property (nonatomic, strong) PullRefreshActivityView *activityView;
@property (nonatomic, assign) CGFloat activityViewScale;
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position dragProgress:(void (^)(CGFloat progress))dragProgressHandler
{
    if(!self.pullToRefreshView) {
        ...
        view.dragProgressHandler = dragProgressHandler;
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    [self addPullToRefreshWithActionHandler:actionHandler position:position dragProgress:NULL];
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        PullRefreshActivityView *pullRefreshActivityView = [[PullRefreshActivityView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
        self.activityView = pullRefreshActivityView;
        self.activityView.center = CGPointMake([UIScreen mainScreen].bounds.size.width/2., self.frame.size.height/2. + 5);
    ...
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        if ((self.state != SVPullToRefreshStateLoading)) {
            ...
            if (contentOffset.y > 0) {
                ...
                progress = contentOffset.y / scrollOffsetThreshold;
            }
            
            if (self.dragProgressHandler)
                self.dragProgressHandler(progress);
            
            self.activityView.progress = progress;
            self.activityView.center = CGPointMake(self.activityView.center.x, 60 - 50 * progress / 2.);
            self.activityView.layer.transform = CATransform3DMakeScale(progress, progress, progress);
        }
    } else {
        CGFloat offset;
    ...
    switch (newState) {
        ...
        case SVPullToRefreshStateLoading:
            ...
            if(previousState == SVPullToRefreshStateTriggered && pullToRefreshActionHandler) {
                ...
                if (self.scrollView.infiniteScrollingView) {
                    [self.scrollView.infiniteScrollingView setNeedRefreshState:YES];
